import asyncdispatch, options, strutils, strformat
import dimscord
import typedefs, logger

using
    s: Shard
    i: Interaction

import slashprocs

#[
ApplicationCommand(
    name: "help",
    description: "Provides general help for the bot.",
    kind: atSlash,
    default_permission: true
)
]#
proc getApplicationCommandList*(): seq[ApplicationCommand] =
    for command in SlashCommandList:
        result.add(ApplicationCommand(
            name: command.name,
            description: command.desc,
            kind: command.kind,
            options: command.options,
            default_permission: not command.specialPermission
        ))

proc handleSlashInteraction*(s, i): Future[system.void] {.async.} =
    let data = i.data.get()

    # Get requested command:
    var command: SlashCommand
    for cmd in SlashCommandList:
        if cmd.name == data.name:
            command = cmd
            break

    # No command found (should *normally* not happen):
    if command.name == "":
        await discord.api.interactionResponseMessage(
            i.id, i.token,
            kind = irtChannelMessageWithSource,
            response = SlashResponse(
                content: &"The requested command '{data.name}' was not found. This is most likely an internal error."
            )
        )
        return

    # Check if server-only command:
    if i.member.isSome() and command.serverOnly:
        await discord.api.interactionResponseMessage(
            i.id, i.token,
            kind = irtChannelMessageWithSource,
            response = await sendErrorMessage(s, i, USAGE, "This command can only be executed on servers.")
        )
        return

    # Call command:
    try:
        await discord.api.interactionResponseMessage(
            i.id, i.token,
            kind = irtChannelMessageWithSource,
            response = await command.call(s, i)
        )
    # Catch runtime errors:
    except Exception as e:
        logger e
        await discord.api.interactionResponseMessage(
            i.id, i.token,
            kind = irtChannelMessageWithSource,
            response = await sendErrorMessage(s, i, INTERNAL, &"**{e.name}**\n{e.msg}")
        )
        return


# Add commands:

proc add(command: SlashCommand) =
    SlashCommandList.add(command)

add SlashCommand(
    name: "help",
    desc: "Provides general information about the bot.",
    kind: atSlash,
    call: helpSlash
)

add SlashCommand(
    name: "echomessage",
    desc: "Echoes back anything that you say!",
    kind: atSlash,
    options: @[SlashOption(
        kind: acotStr,
        name: "message",
        description: "This string will be echoed.",
        required: some true
    )],
    call: echoSlash
)
